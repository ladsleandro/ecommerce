...
{
    const SESSION = "User";
    const SECRET = "HcodePhp7_Secret";//"LadsDevWebSecret";
    const SECRET_IV = "HcodePhp7_Secret_IV";//"LadsDevWeb_Secret_IV";
    ...
    public static function getForgot($email, $inadmin = true)
    {
        ...
        if (count($results) === 0) {
            ...
            if (count($recovery) === 0) {
                ...
                if ($inadmin) {
                    $link = "http://www.ladsecommerce.com.br/admin/forgot/reset?code=$code";
                } else {
                    $link = "http://www.ladsecommerce.com.br/forgot/reset?code=$code";
                }

                $mailer = new Mailer($data['desemail'], $data['desperson'], "Redefinir senha da Lads Store", "forgot", array(
                    "name" => $data['desperson'],
                    "link" => $link
                ));
                $mailer->send();
